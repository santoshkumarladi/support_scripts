...
RWMIXREADS=(50 30)
BSSPLIT='1K/10:4K/30:8K/40:32K/10'
gen_job_file() 
{
    ...
    echo  "overwrite=1" >> $job
    echo  "do_verify=1" >> $job
    echo  "iodepth_batch=2" >> $job
    echo  "iodepth_batch_complete=2" >> $job
    echo  "verify_fatal=1" >> $job
    echo  "verify_dump=1" >> $job
    echo  "time_based" >> $job
    #echo  "rate=300m,1000K" >> $job
    echo  "rate_iops=3" >> $job
    echo  "buffer_compress_percentage=50" >> $job
    echo  "refill_buffers" >> $job
           #"--buffer_pattern=shivayogidavanageri",
    echo  "verify=sha1" >> $job
    echo  "numjobs=8"   >> $job
    echo  "size=$SIZE"   >> $job
    echo  "bssplit=$BSSPLIT"   >> $job
