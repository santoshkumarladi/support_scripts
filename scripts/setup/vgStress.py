...
def cofigClient(dest,cvm,pat,vgName,image):
    ...
    #print cmd
    #os.system(cmd)
    cmd = ssh+" vg.list | grep "+str(vgName)
    print cmd
    output = (os.popen(cmd).read()).split('\n')
    for i in range(0,len(output)-1):
        (vg,uuid) = output[i].split()
        cmd = acli+" vg.attach_external "+vg+" initiator_name="+iqn
        print cmd
        os.system(cmd)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hc:g:i:l:f:d:r:p:o:w:",["cvm=","vgm=","init=","dest=","fsize=","dsize=","onlyS=","pat=","onlyreg=","wait="])
