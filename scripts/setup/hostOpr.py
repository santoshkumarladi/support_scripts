...
def get_svmips(cvm):
    ...
    if op :
        ...
        for i in range(1,len(output)):
            #10.40.225.132  10.40.225.132        fa82581e-7578-4966-bbfb-b964e55ba6c5  AcropolisNormal  False      Hyperconverged  False        AHV              10.40.225.135
            if output[i].split()[3] == 'AcropolisNormal' :
            #if output[i].split()[3] == 'AcropolisNormal' and output[i].split()[6] == 'True':
...
def ctrPropChange(cvm,ctrs):
    ...
    #Error: Fingerprint on write cannot be enabled when the feature is disabled in the cluster
    cmd = ssh+ncli+"container edit name="+str(ctrn)+" fingerprint-on-write=on placement-policy-name=rf3_1n2d"
    print cmd
    p = re.compile('Error:\s+Fingerprint\s+on\s+write\s+cannot\s+be\s+enabled\s+when\s+the\s+feature\s+is\s+disabled\s+in\s+the\s+cluster')
    ma = p.match((os.popen(cmd).read()))
    if ma is not None:
        no_dedupe = 1
        print "Dedupe is not enabled on the cluster " 
    ...
    for ctr in ctrs.keys():
        ...
        if comp_delay and enable_compression == 'true':
            if rf == 3:
                cmd = ssh+ncli+"container edit name="+str(ctr)+" "+ec+" enable-compression="+str(enable_compression)+" on-disk-dedup="+str(odp)+" compression-delay="+str(comp_delay)+" fingerprint-on-write="+str(fingerprint_on_write)+" rf="+str(rf)+" placement-policy-name=rf3_1n2d"
            else :
            ...
            if rf == 3:
                cmd = ssh+ncli+"container edit name="+str(ctr)+" erasure-code="+str(erasure_code)+" enable-compression="+str(enable_compression)+" on-disk-dedup="+str(odp)+" fingerprint-on-write="+str(fingerprint_on_write)+" rf="+str(rf) +" placement-policy-name=rf3_1n2d"
            else : 
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hi:t:d:w:u:f:s:p:o:m:x:",["cvm=","tier=","dtype=","wait=","tusuage=","diskflt=","hostfail=","pattern=","diskoff=","migratec=","passwd="])
...
def sshCopy(ip,passwd):
    ...
    try :
        ...
        print "Oops Something went wrong buddy "+passwd
        print e
