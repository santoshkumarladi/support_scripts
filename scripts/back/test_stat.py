...
class stats_collector(object):
  ...
  "disk_queue_stats" : "2009/disk_queue_stats", "vdisk_stats" : "2009/vdisk_stats"}
  ...
  def run_ssh_cmd(self, cmd):
    try:
      password="nutanix/4u"
      pkey_path = "/home/nutanix/.ssh/id_rsa_copy"
      key = RSAKey.from_private_key_file(pkey_path,password)
      self.ssh_client.connect(self.virtual_ip, port=22, username="nutanix", password="nutanix/4u", pkey=key)
      #self.ssh_client.connect(self.virtual_ip, port=22, username="nutanix", password="nutanix/4u")
