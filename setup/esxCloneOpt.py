...
"""
esxCloneOpt.py a Python script which allows you to clone a virtual machine or virtual machine template into multiple new virtual machines in a VMware vSphere environment.
...
--- Usage ---
Run 'esxCloneOpt.py -h' for an overview
...
def get_args():
    ...
    parser.add_argument('-p', '--password', nargs=1, required=False, help='The password with which to connect to the host. If not specified, the user is prompted at runtime for a password', dest='password', default=['Nutanix/4u'], type=str)
    ...
    parser.add_argument('-u', '--user', nargs=1, required=False, help='The username with which to connect to the host', dest='username',  default=['administrator@vsphere.local'] ,type=str)
...
def main():
    ...
    try:
        ...
        if not si:
            logger.error('Could not connect to host %s with user %s and specified password %s' % (host, username, password))
            return 1
