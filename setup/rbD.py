...
def delVmbyPattern(cvm,vname):
    ...
    for i in range(1,len(output)-1):
        ...
        print cmd
        os.system(cmd)
        ncli = " /ihome/nutanix/prism/cli/ncli "
        runs = ssh +ncli
...
def runTheScript(ip,uuid,mntp,userid,passwd,b,d):
    ...
    run = "ssh "+ip+" -l "+userid+" \"sudo /home/nutanix/scripts/setup/rebuildDisk.py -m " +mntp+ " -b "+str(b)+" -d "+str(d)+" </dev/null>/dev/null 2>&1 &\""
    print run
    os.system(run)
