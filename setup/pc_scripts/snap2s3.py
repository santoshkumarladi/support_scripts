...
def snap2workflow(connection,uuid):
    print(connection.protection_status(uuid)["data"])
    while 1:
        if "PROTECTED" != (connection.protection_status(uuid))["data"]["protectionState"] and \
                "PROTECTED" != (connection.protection_status(uuid))["data"]["targetProtectionState"]:
            print(f"Enable protection on the cluster -> {uuid}")
            connection.enable_protection(uuid)

            time.sleep(60)
            #print((connection.protection_status(uuid))["data"]["data"]["currentProtectionState"])
            while "PROTECTED" != (connection.protection_status(uuid))["data"]["protectionState"]:
                status = (connection.protection_status(uuid))["data"]["protectionState"]
                print(f"PROTECT:Cluster is in -> {status} -> {uuid}")
                time.sleep(60)

        if "PROTECTED" == (connection.protection_status(uuid))["data"]["protectionState"] and \
                "PROTECTED" == (connection.protection_status(uuid))["data"]["targetProtectionState"]:
            connection.disable_protection(uuid)
            while "UNPROTECTED" != (connection.protection_status(uuid))["data"]["protectionState"]:
                status = (connection.protection_status(uuid))["data"]["protectionState"]
                print(f"UNPROTECT: Cluster is in -> {status} -> {uuid}")
                time.sleep(60)
        else :
            print(f"Protection status -> {(connection.protection_status(uuid))['data']['protectionState']}")
            time.sleep(600)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hc:u:p:s:b:i:",["pc=","user=","passwd=","sp=","batch=","ignore="])
    ...
    clusters_list = (connection.get_clusters(cluster_payload))["entities"]
    for cls in clusters_list:
        if "PC" not in   cls["spec"]["name"] and cls["status"]["name"] != ignore :
            clusters.append(cls["metadata"]["uuid"])
            print (cls["metadata"]["uuid"])
    ...
    for uuid in clusters:
        thread = threading.Thread(target=snap2workflow, args=(connection,uuid,))
