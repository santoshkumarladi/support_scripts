...
direct=1
if [[ "$7" = "zfs" ]]; then
  disk_list=/zfs/file_for_seq_write_test
...
taskset 0x1 \
sudo /usr/bin/fio  --write_bw_log=$fio_log_prefix --write_iops_log=$fio_log_prefix --write_lat_log=$fio_log_prefix --log_avg_msec=1000 --filename=$disk_list --direct=$direct --rw=write --bs=$4 --iodepth=$3 --size=$(($1*$2))  --runtime=$5 $time_based --group_reporting --name=file1 --ioengine=$io_engine 
