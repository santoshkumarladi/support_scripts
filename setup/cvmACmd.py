...
def createSnapshot (ssh,vmlist,count,wait):
...
def runS(cvm,userid,passwd):
    ...
    t = threading.Thread(target=createSnapshot, args=(ssh,vmlist,count,wait,))
    threads.append(t)
    t = threading.Thread(target=delSnapshot, args=(ssh,vmlist,))
    threads.append(t)
